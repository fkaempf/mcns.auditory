...
lb0_male <- mba%>%
  filter(itolee_hl == "LB0") %>% 
  select(bodyid) %>% pull() %>% 
  as.character()

baker_fw <- baker_neurons %>% 
  filter(grepl("WV", neuron_name)) %>%
  select(root_783) %>%
  pull()

lb0neurons_fw_meta=cf_meta(cf_ids(flywire = baker_fw))
lb0neurons_fw_meta <- lb0neurons_fw_meta %>% 
  select(id, type, ito_lee_hemilineage, malecns_type, instance) %>% 
  rename(bodyid = "id", itolee_hl = "ito_lee_hemilineage", flywire_type = "type")

lb0neurons_mcns_meta <- mba %>% 
  filter(itolee_hl == "LB0") %>% 
  select(bodyid, type, itolee_hl, flywire_type, instance) %>% 
  rename(malecns_type = "type")
...
name_of_sheet = 'WV_WV'
#googlesheets4::write_sheet(data = lb0neurons_all_info1, 
#                           ss= "https://docs.google.com/spreadsheets/d/1PMcHcy99lgJi_2Sk4msTwKfTs7NcK7fVGz40qLiXd1k/edit?usp=sharing",
#                           sheet=name_of_sheet)