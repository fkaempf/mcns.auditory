...
#get the feather file of connectivity here for all important bodies (keep working with this until there is a new version from Stuart):
#https://storage.googleapis.com/flyem-cns-z0720_07m_br-40-06-derived/brain-connectome-exports-2023-11-15-f20f31-unlocked/connectome-weights-2023-11-15-f20f31-unlocked-minconf-0.5-primary-only.feather
#withr::with_dir("~/Desktop/DNproject/maleCNS/plotting/",
...
# scale columns while staying sparse ... this calculates input percent for all neurons in the matrix
# There is a nice function for this in 2021manc/R/mancfuncs/manc/sparsemat.R, dropping it below for people without access to this repo
...
#for example some query or a list of IDs (example here gustatory sensory neurons)
#input_ids <- mcns_ids("/type:ORN_DA1")
#input_ids <- male_wedge$bodyid
#input_ids <- mcns_ids("/type:LC.*")
...
#input_ids <- mcns_ids("/type:vpo.*")
#input_ids <- mcns_ids("/type:LC10.*")
#input_ids <- mcns_ids("/subclass:taste peg")
#input_ids <- all_info$bodyid[grepl("gustatory",all_info$cell_class)]
#all.fru <- all.fru %>% rename(bodyid = id)
#input_ids <- all.fru %>% filter(sex == "M" & fru == "Y") %>% select(bodyid) %>% pull()
#input_ids <- all_info$bodyid[grepl("gustatory|olfactory", all_info$cell_class) | grepl("optic", all_info$class)]
input_info = all_info[all_info$bodyid %in% input_ids,]
#all.fru <- all.fru %>% mutate(bodyid = as.integer(bodyid))
#input_info <- input_info %>% left_join(all.fru, by = "bodyid")
#input_info_sel <- input_info[,c("bodyid","class","group","type","instance","soma_side", "root_side", "cell_class", "clone")]
...
#input_info_sel$cell_class <- ifelse (input_info_sel$class == "optic", "optic", input_info_sel$cell_class)

#get your output IDs and info from eiclio
#for example some query or a list of IDs (example here DNa neurons)
# output_ids <- c(10090, 10104, 10360, 10379, 10442, 10460, 10543, 10672, 10760, 10837, 10884, 10924, 10971, 10975, 11002, 11066, 11233, 11258, 11314, 11392, 11563, 11610, 11625, 11739)
#output_ids <- mcns_ids("/type:pC1x_.*")
#output_ids <- mcns_ids("/type:P1_.*")
...
#output_ids <- c(output_ids, "10030", "10038")
output_info = all_info[all_info$bodyid %in% output_ids,]
#output_ids <- all.fru %>% filter(sex == "M" & fru == "Y") %>% select(bodyid) %>% pull()
#output_info <- output_info %>% left_join(all.fru, by = "bodyid")
#output_info_sel <- output_info[,c("bodyid","class","group","type","instance","soma_side", "root_side", "cell_class", "clone")]
