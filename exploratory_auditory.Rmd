...
#all.auditory.output$type <- all.auditory.output.match$match.type.f2m
...
Make a connectitiy matrix for the circuit

```{r}
cf_partner_summary(cf_ids(malecns='flywireType:CB1078'),threshold=5,partners = 'out')%>%
  mutate(weight_normed_by_post_n=weight/npost)%>%
  arrange(desc(weight_normed_by_post_n))%>%
  View()
```
